...
def train_inversion_model_mlc(train_dataloader, eval_dataloader=None, inversion_epochs=5, inversion_lr=1e-4, device='cuda', output_dir=None):
    ...
    for epoch in range(inversion_epochs):
        for step, batch in enumerate(train_dataloader):            
            ...
            target_label[word_filter(target_label, filter_tokens)]=0
        ...
        torch.save(inversion_model, f'{output_dir}/inversion_model_mlc_ft.pt')
        print(f'save inversion model for ft in {output_dir}/inversion_model_mlc_ft.pt')
    return inversion_model
                
def evaluate_inversion_model_mlc(inversion_model, eval_dataloader, tokenizer, task_type='sentence', device='cuda'):
    special_tokens = [tokenizer.pad_token, tokenizer.cls_token, tokenizer.sep_token]
    metric_list = ['token_hit_mlc', 'rouge']
    if task_type == 'token':
    ...
    for batch in eval_dataloader:
        ...
        temp_hit, temp_total = token_hit_multi_label(target_label, preds, tokenizer, special_tokens)
        # temp_hit, temp_total = token_hit(target_label[i].unsqueeze(0),  preds[i].unsqueeze(0), tokenizer, special_tokens)
        metric_counter.update('token_hit_mlc', temp_hit, temp_total)
        progress_bar.update(1)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_name_or_path",
        type=str,
        default='/root/TextFusion_special_attack/save_models/token/conll2003/phase2/5e-05/epoch_3',
    )
    parser.add_argument(
        "--target_layer",
        type=int,
    ...
    print('########### inversionBertMLC attack in {} {} #################'.format(args.dataset_name, args.model_name_or_path))
    ...
    attack_results = evaluate_inversion_model_mlc(inversion_model, eval_dataloader=textfusion_eval_dataloader, tokenizer=tokenizer, task_type=args.task)
