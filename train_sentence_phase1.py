...
def parse_args():
    ...
    parser.add_argument(
        ...
        default='./save_models/sentence/sst2/ft'
        # default='/root/privacy_ner/save_models/baseline/fine-tune/sentence/sst2',
        # default='/root/privacy_ner/save_models/bert-base-cased/sentence/sst2/pooling_ft'
    ...
    parser.add_argument("--output_dir", type=str, default='./save_models/sentence/sst2/phase1', help="Where to store the final model.")
...
def main():
    ...
    if args.use_wandb:
        wandb.init(sync_tensorboard=False,
                project="textfusion_phase1_{}_layer{}".format(args.task_name, args.target_layer),
                job_type="CleanRepo",
                config=args,
                name=item_name
                )
    ...
    if args.task_name is not None:
        ...
        # raw_datasets = datasets.load_from_disk('./datasets/{}'.format(args.task_name))
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        if args.use_wandb:
            wandb.log({"dev/{}".format(target_metric): eval_metric[target_metric]}, step=completed_steps)
        logger.info(f"epoch {epoch}: {eval_metric}")
        # if eval_metric[target_metric]<0.6 and total_loss/(step+1) > 0.6:
        #     break
        if eval_metric[target_metric] >= best_metric:
            best_metric = eval_metric[target_metric]
            best_metric_dict=eval_metric
            best_epoch = epoch
            if args.output_dir is not None:
                if os.path.exists(os.path.join(args.output_dir, "all_results.json")):
                    with open(os.path.join(args.output_dir, "all_results.json"), "r") as f:
                        ...
                        if best_metric > global_best_metric['eval_{}'.format(target_metric)]:
                            save_model = True
                        else:
                            save_model = False
            if save_model:
                ...
                with open(os.path.join(args.output_dir, "all_results.json"), "w") as f:
                    json.dump({"eval_{}".format(target_metric): eval_metric[target_metric], "lr":args.learning_rate}, f)

    with open('./logs/sentence_textfusion.txt', 'a') as f:
        f.write('phase1 seed:{}, init model: {}, task:{}, lr:{}, epoch:{},  performance:{} \n'.format(args.seed, args.model_name_or_path, args.task_name, args.learning_rate, best_epoch, best_metric_dict))
    if args.use_wandb:
        wandb.finish()
